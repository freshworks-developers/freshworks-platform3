...
function findProjectRoot(targetDir) {
  // If target directory provided, use it
  if (targetDir) {
    const resolved = path.resolve(targetDir);
    if (fs.existsSync(resolved)) {
      return resolved;
    } else {
      log(`⚠️  Target directory not found: ${targetDir}`, COLORS.yellow);
      log(`   Using current directory instead: ${process.cwd()}`, COLORS.yellow);
    }
  }
  ...
  while (currentDir !== path.parse(currentDir).root) {
    ...
    const hasCursorFreeFolder = fs.existsSync(path.join(currentDir, '.cursor-free'));
    
    if (hasPackageJson || hasGitFolder || hasCursorFolder || hasCursorFreeFolder) {
      return currentDir;
    }
...
function installSkill(targetDir) {
  ...
  // Find the target project directory (use provided target or detect)
  ...
  // Install for Cursor IDE (both paid and unpaid)
  try {
    ...
    const commandsSource = path.join(skillSourceDir, '.cursor', 'commands.json');
    
    // Detect Cursor type
    const hasCursorFree = fs.existsSync(path.join(projectRoot, '.cursor-free'));
    const hasCursor = fs.existsSync(path.join(projectRoot, '.cursor'));
    
    // Determine which directory to use (prefer .cursor-free if exists, otherwise .cursor)
    const cursorDir = hasCursorFree ? '.cursor-free' : '.cursor';
    
    if (fs.existsSync(cursorRulesSource)) {
      log('\n📋 Installing Cursor Rules...', COLORS.yellow);
      const cursorRulesTarget = path.join(projectRoot, cursorDir, 'rules');
      copyDirectory(cursorRulesSource, cursorRulesTarget);
      log(`   ✓ Installed to ${cursorDir}/rules/`, COLORS.green);
      installCount++;
    }
    
    if (fs.existsSync(cursorSkillsSource)) {
      log('📋 Installing Cursor Skills...', COLORS.yellow);
      const cursorSkillsTarget = path.join(projectRoot, cursorDir, 'skills');
      copyDirectory(cursorSkillsSource, cursorSkillsTarget);
      log(`   ✓ Installed to ${cursorDir}/skills/`, COLORS.green);
      installCount++;
    }
    
    // Install commands.json if it exists
    if (fs.existsSync(commandsSource)) {
      log('⚡ Installing Slash Commands...', COLORS.yellow);
      const commandsTarget = path.join(projectRoot, cursorDir, 'commands.json');
      fs.copyFileSync(commandsSource, commandsTarget);
      log(`   ✓ Installed to ${cursorDir}/commands.json`, COLORS.green);
    }
  } catch (error) {
...
if (require.main === module) {
  try {
    ...
    // If no target provided, installs to current working directory
