...
function installSkill() {
  ...
  if (isGlobal) {
    log('\n🌍 Installing globally...', COLORS.bright + COLORS.blue);
  } else {
    log('\n📦 Installing to current project...', COLORS.bright + COLORS.blue);
  }
  ...
  if (isGlobal) {
    ...
    log(`📁 Global install: ${projectRoot}`, COLORS.blue);
  } else {
  ...
  try {
    ...
    if (fs.existsSync(cursorRulesSource)) {
      ...
      // For global install, only install rules (not skills)
      if (isGlobal) {
        // Try both .cursor and .cursor-free for global
        const cursorRulesTarget = path.join(projectRoot, '.cursor', 'rules');
        const cursorFreeRulesTarget = path.join(projectRoot, '.cursor-free', 'rules');
        ...
        try {
          ...
          log(`   ⚠ Could not install to .cursor: ${err.message}`, COLORS.yellow);
        }
        
        // Also try .cursor-free
        try {
          copyDirectory(cursorRulesSource, cursorFreeRulesTarget);
          log('   ✓ Installed to ~/.cursor-free/rules/', COLORS.green);
          installCount++;
        } catch (err) {
          // Silent fail for .cursor-free if it doesn't exist
        }
      } else {
        ...
        // Check for .cursor-free and install there too
  ...
  if (installCount > 0) {
    ...
    if (isGlobal) {
      log('   3. Rules will be available in ALL projects!', COLORS.green);
    } else {
      log('   3. Rules will be available in this project only', COLORS.reset);
      log('\n💡 Tip: Use --global flag to install for all projects:', COLORS.yellow);
      log('   node scripts/install.js --global', COLORS.blue);
    }
  } else {
