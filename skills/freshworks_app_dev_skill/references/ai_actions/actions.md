...
5. **Where**: Defined in actions.json(<app root>/actions.json), the invocation would happen from the flow consuming the actions.json specifications.
6. **Invocation**: 
...
1. Modify `actions.json`(<app root>/actions.json) and specify the function name and parameters to invoke the action flow.
...
In `server.js`, define the callback method (action) that is defined in `actions.json`(<app root>/actions.json) and called from the app flow. In this callback method, include the app logic that runs based on the payload passed and the `renderData()` method to return success and failure responses.
